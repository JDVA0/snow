...
func TestBlizzardShowcaseProgram(t *testing.T) {
	src, err := os.ReadFile(filepath.Join("testdata", "programs", "blizzard_showcase.blizz"))
	if err != nil {
		t.Fatal(err)
	}
	out, err := run(t, string(src))
	if err != nil {
		t.Fatalf("showcase failed: %v", err)
	}
	for _, want := range []string{"Blizzard 2 fallback", "[20,40,60]", "small", "expected failure", "cleanup"} {
		if !strings.Contains(out, want) {
			t.Fatalf("showcase output %q does not contain %q", out, want)
		}
	}
}

func TestBraceSyntaxStress(t *testing.T) {
	valid := []string{
		`fn f() {
    let data = {
        outer: {inner: [1, 2, 3]}
    ...
    print(double(data.outer.inner[1]))
}
f()`,
		`let total = 0
...
always {
	...
	for index, src := range valid {
		src = strings.ReplaceAll(src, "\t", "")
		if _, err := run(t, src); err != nil {
			t.Errorf("valid stress case %d failed: %v", index, err)
		}
		formatted, err := Format(src)
		if err != nil {
			t.Errorf("format stress case %d failed: %v", index, err)
			continue
		}
		again, err := Format(formatted)
		if err != nil {
			t.Errorf("format second pass %d failed: %v", index, err)
			continue
		}
		if formatted != again {
			t.Errorf("format stress case %d is unstable:\n%s\n---\n%s", index, formatted, again)
		}
	}
}

func TestBraceSyntaxRejectsMalformedPrograms(t *testing.T) {
	cases := []string{
		"fn missing() {",
		"if true { print(1) ",
		"fn bad() { let x = 1 } }",
		"match 1 { case _ { print(1) } case 2 { print(2) } }",
	}
	for _, src := range cases {
		if _, err := Parse(src, "malformed.blizz"); err == nil {
			t.Errorf("expected malformed source to fail:\n%s", src)
		}
	}
}
...
func TestStringsAndLists(t *testing.T) {
	...
	check(t, `print(upper("blizzard"))`, "BLIZZARD")
	check(t, `print(join(split("a,b,c", ","), "-"))`, "a-b-c")
	check(t, `print(contains("blizzard", "blizzard"))`, "true")
...
func TestStringMethods(t *testing.T) {
	check(t, `
print("blizzard".upper(), "BLIZZARD".lower(), "  blizzard  ".trim())
print("a,b,c".split(",").join("-"))
print("banana".replace("na", "x"), "banana".contains("nan"))
print("blizzard".starts_with("bl"), "blizzard".ends_with("rd"), "banana".count("an"), "banana".index_of("na"), "ha".repeat(3))
`, "BLIZZARD blizzard blizzard\na-b-c\nbaxx true\ntrue true 2 2 hahaha")
}

func TestLengthMethod(t *testing.T) {
	check(t, `print(["manzana", "pera"].length(), "blizzard".length(), {a: 1}.length())`, "2 8 1")
}
...
func TestSysAndFS(t *testing.T) {
	check(t, `using sys
res = sys.sh("echo hello-blizzard")
print(trim(res.stdout), res.ok)`, "hello-blizzard true")

	check(t, `using sys
sys.set_env("BLIZZARD_VAR", "blizzard_val_123")
print(sys.env("BLIZZARD_VAR"))`, "blizzard_val_123")

	check(t, `using fs
test_file = "temp_test_blizzard.txt"
...
func TestCLIModule(t *testing.T) {
	check(t, `using cli
p = cli.parse(["--port", "8080", "--verbose", "serve", "app.blizz"])
print(p.flags.port, p.flags.verbose, p.args[0], p.args[1])`, "8080 true serve app.blizz")
...
`
	...
	req = httptest.NewRequest("POST", "/echo", strings.NewReader(`{"name":"blizzard"}`))
	req.Header.Set("Content-Type", "application/json")
	w = httptest.NewRecorder()
	s.handle(w, req)
	if w.Code != 201 || !strings.Contains(w.Body.String(), `"name":"blizzard"`) {
...
func TestCLIBox(t *testing.T) {
	testCases := []string{
		`
using cli
cli.box("❄️ BLIZZARD CLI TOOLKIT", "A modern, concise & elegant CLI experience\nPlatform: linux (amd64) | PID: 76365")
`,
		`
using cli
cli.box("hola", "mundo")
...
func TestJSONModule(t *testing.T) {
	src := `
using json
obj = json.parse('{"name": "blizzard", "count": 10, "active": true}')
print(obj.name)
print(obj.count)
print(obj.active)
...
# JWT
token = crypto.jwt_sign({"user": "blizzard", "role": "admin"}, "secret123")
...
`, "anonymous")
	...
	check(t, `
data = {"user": {"profile": {"name": "blizzard"}}}
print(data?["user"]?["profile"]?["name"] ?? "anonymous")
`, "blizzard")
...
func TestEnvModule(t *testing.T) {
	check(t, `
using env
env.set("BLIZZARD_TEST_VAR", "hello_blizzard")
print(env.get("BLIZZARD_TEST_VAR"))
print(env.has("BLIZZARD_TEST_VAR"))
print(env.get("BLIZZARD_NON_EXISTENT", "default_val"))
`, "hello_blizzard\ntrue\ndefault_val")

	check(t, `
using env
env.set("BLIZZARD_TEST_NUM", "42")
env.set("BLIZZARD_TEST_BOOL", "true")
print(env.int("BLIZZARD_TEST_NUM"))
print(env.bool("BLIZZARD_TEST_BOOL"))
print(env.int("BLIZZARD_TEST_MISSING", 100))
`, "42\ntrue\n100")
}
...
func TestBraceSyntaxWithLetAndImport(t *testing.T) {
	check(t, strings.ReplaceAll(`import json;
let value = 2;
fn twice(number) {
	if (number > 1) {
...
func TestModernFormatIsStable(t *testing.T) {
	src := `import json; let value=1; fn show(number) { if(number > 0) { print(json.stringify(number)); } else { print("small"); } } show(value);`
	out, err := Format(src)
	if err != nil {
		t.Fatalf("Format: %v", err)
	}
	want := "import json;\nlet value = 1;\nfn show(number) {\n    if (number > 0) {\n        print(json.stringify(number));\n    } else {\n        print(\"small\");\n    }\n}\nshow(value);\n"
	if out != want {
		t.Fatalf("Format got:\n%s\nwant:\n%s", out, want)
	}
	again, err := Format(out)
...
func TestFormatModernMigratesLegacyBlocks(t *testing.T) {
	src := "fn add(a, b):\n    result = a + b\n    return result\n"
	out, err := FormatModern(src)
	if err != nil {
		t.Fatal(err)
	}
	want := "fn add(a, b) {\n    let result = a + b;\n    return result;\n}\n"
	if out != want {
		t.Fatalf("modern format = %q, want %q", out, want)
	}
}
...
func TestEnvIntInvalid(t *testing.T) {
	check(t, `
using env
env.set("BLIZZARD_BAD_INT", "abc")
try:
    env.int("BLIZZARD_BAD_INT")
    print("no")
catch err:
    print("bad")
print(env.int("BLIZZARD_BAD_INT", 7))
`, "bad\n7")
}
...
`,
		...
		if _, err := Parse(src, "test.blizz"); err == nil {
...
for left, right in zip([1, 2, 3], ["a", "b"]):
    print(left, right)
`, "0 [0, a]\n1 [1, b]\n0 [1, a]\n1 [2, b]")
	if _, err := run(t, `enumerate("blizzard")`); err == nil || !strings.Contains(err.Error(), "enumerate expects a list") {
...
func TestFormatDiagnostic(t *testing.T) {
	src := "value = 1\nprint(missing)\n"
	err := New().Run(src, "demo.blizz")
	got := FormatDiagnostic(err, src)
	if !strings.Contains(got, "demo.blizz:2:") || !strings.Contains(got, "2 | print(missing)") || !strings.Contains(got, "^") {
...
func TestDiagnosticCodes(t *testing.T) {
	...
	if got := CodeFor(&Errat{File: "x.blizz", Line: 1, Col: 1, Err: fmt.Errorf("module 'x' not found")}); got != CodeModule {
		t.Fatalf("module code = %s, want %s", got, CodeModule)
	}
	issues, err := Check("print(missing)\n", "codes.blizz")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAssertDiagnosticPointsAtCall(t *testing.T) {
	src := "fn main():\n    assert(1 == 2)\n\nmain()\n"
	err := New().Run(src, "assert.blizz")
	got := FormatDiagnostic(err, src)
	if !strings.Contains(got, "assert.blizz:2:5:") || !strings.Contains(got, "assertion failed: condition evaluated to false") {
...
func TestMissingBlockErrorPointsAtColon(t *testing.T) {
	...
	if !strings.Contains(msg, "block.blizz:2:") {
...
func TestUnknownModuleSuggestion(t *testing.T) {
	_, err := run(t, "using blizzard.htp\n")
	if err == nil || !strings.Contains(err.Error(), "did you mean 'blizzard.http'?") {
...
func TestCheckGradualTypes(t *testing.T) {
	issues, err := Check(`age: int = "old"`, "types.blizz")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCheckUnknownModule(t *testing.T) {
	issues, err := Check("using blizzard.unknown\n", "a.blizz")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	var hasUnknown bool
